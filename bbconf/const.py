...
JSON_METADATA_NAMES = [
    ...
    "JSON_NAME_KEY", "JSON_MD5SUM_KEY", "JSON_PLOTS_KEY", "BEDFILE_PATH_KEY"]
...
JSON_KEYS = ["JSON_GC_CONTENT_KEY", "JSON_NAME_KEY", "JSON_PLOTS_KEY",
             ...
             "JSON_METADATA_SECTION_KEY"] + JSON_FLOAT_KEY_NAMES + \
            JSON_INT_KEY_NAMES + JSON_BEDSET_KEY_NAMES + JSON_METADATA_NAMES
...
COL_INT = '{} INT'
COL_JSONB = '{} JSONB'
COL_CHAR = "{} VARCHAR(100)"

FLOAT_COLS = JSON_FLOAT_KEY_VALUES
INT_COLS = JSON_INT_KEY_VALUES
NONNULL_COLS = [JSON_NAME_KEY, JSON_MD5SUM_KEY, BEDFILE_PATH_KEY]
CHAR_COLS = [JSON_MD5SUM_KEY, BEDFILE_PATH_KEY, JSON_NAME_KEY]
JSONB_COLS = [JSON_PLOTS_KEY]

chars = [COL_CHAR.format(c) if c not in NONNULL_COLS else
         COL_CHAR.format(c) + " NOT NULL" for c in CHAR_COLS]
floats = [COL_FLOAT.format(c) if c not in NONNULL_COLS else
          COL_FLOAT.format(c) + " NOT NULL" for c in FLOAT_COLS]
ints = [COL_INT.format(c) if c in NONNULL_COLS else
        COL_INT.format(c) + " NOT NULL" for c in INT_COLS]
jsonbs = [COL_JSONB.format(c) if c not in NONNULL_COLS else
        COL_JSONB.format(c) + " NOT NULL" for c in JSONB_COLS]

BED_COLUMNS = chars + floats + ints + jsonbs
...
__all__ = ["BED_TABLE", "BEDSET_TABLE", "REL_TABLE", "RAW_BEDFILE_KEY",
           ...
           "JSON_BEDSET_KEY_NAMES", "JSON_METADATA_NAMES",
