...
def aggregate_collection(
    ...
    stats = BedSetStats(
        ...
        tss_histogram=_aggregate_fixed_axis_from_dists(distributions_list, "tss_distances"),
        widths_histogram=_aggregate_variable_histogram(distributions_list, "widths"),
        neighbor_distances=_aggregate_variable_kde(distributions_list, "neighbor_distances"),
    ...
    if precision is not None:
        stats = BedSetStats(**round_floats(stats.model_dump(), precision))
...
def _aggregate_composition(rows: list) -> Optional[dict]:
    ...
    Returns {field: {value: count}} for genome_alias, assay, cell_type, tissue, target.
    """
    if not rows:
        return None
...
def _aggregate_scalars(
    ...
    Scalars are: number_of_regions, mean_region_width, median_tss_dist, gc_content.
    Falls back to flat BedStats columns for records without distributions blob.
    """
    scalar_keys = ["number_of_regions", "mean_region_width", "median_tss_dist", "gc_content"]

    # Collect per-file scalar values
    ...
    # Row columns: id, number_of_regions, mean_region_width, median_tss_dist, gc_content
    ...
    for k in scalar_keys:
        ...
        if not vals:
            continue
        arr = np.array(vals)
        hist_counts, hist_edges = np.histogram(arr, bins=min(_SCALAR_HIST_BINS, len(vals)))
        result[k] = {
...
def _aggregate_fixed_axis_from_dists(
    ...
    All files should have the same bin count and axis range. Stack and compute
    element-wise mean + sd.
    """
    arrays = []
    metadata = None
...
def _aggregate_fixed_axis(
    ...
    # Find common length (use the most common length if they differ)
    ...
    result = {
        ...
        "sd": np.std(stacked, axis=0, ddof=1).tolist() if len(filtered) > 1 else [0.0] * target_len,
        "n": len(filtered),
    }
    if metadata:
...
def _aggregate_variable_histogram(
    ...
    Uses CDF interpolation: convert each histogram to a CDF, interpolate onto
    a common x-grid, then differentiate back to get density on the common bins.
    """
    histograms = []
    for dist in distributions_list:
        dists = dist.get("distributions", {})
    ...
    for h in histograms:
        ...
        # Build CDF from original counts
        ...
        # Interpolate CDF at common bin edges
        cdf_interp = np.interp(common_edges, orig_edges, cdf)
        # Differentiate to get counts in each new bin
        new_counts = np.diff(cdf_interp)
        # Normalize so total is preserved
        new_counts = new_counts * (total / new_counts.sum()) if new_counts.sum() > 0 else new_counts
    ...
    result = {
        ...
        "mean": np.mean(stacked, axis=0).tolist(),
        "sd": np.std(stacked, axis=0, ddof=1).tolist() if len(rebinned) > 1 else [0.0] * _COMMON_GRID_SIZE,
        "n": len(rebinned),
    }
    return result
...
def _aggregate_variable_kde(
    ...
    result = {
        ...
        "mean": np.mean(stacked, axis=0).tolist(),
        "sd": np.std(stacked, axis=0, ddof=1).tolist() if len(interpolated) > 1 else [0.0] * _COMMON_GRID_SIZE,
        "n": len(interpolated),
    }
...
def _aggregate_region_distribution(
    ...
    Each file has {chr: [counts_per_bin]}. Pad shorter arrays, stack, mean + sd.
    """
    # Collect all chromosome arrays
    ...
    for chrom, arrays in chrom_data.items():
        ...
        result[chrom] = {
            "mean": np.mean(stacked, axis=0).tolist(),
            "sd": np.std(stacked, axis=0, ddof=1).tolist() if len(padded) > 1 else [0.0] * max_len,
            "n": len(padded),
        }
...
def _aggregate_partitions(
    ...
    """Aggregate genomic partitions across files.

    Per file: convert counts to percentages, then compute mean + sd per category.
    """
    # Collect partition data per file
    file_partitions = []
    for dist in distributions_list:
        ...
        counts = parts.get("counts")  # list of [name, count] pairs
...
def _aggregate_chromosome_stats(
    ...
    Group by chromosome, compute mean + sd of numeric fields.
    """
    chrom_data: Dict[str, List[dict]] = defaultdict(list)
    for dist in distributions_list:
        # chromosome_stats can live at dist["chromosome_stats"] (test fixture)
        # or dist["distributions"]["chromosome_stats"] (real gtars output)
        chrom_stats = dist.get("chromosome_stats") or dist.get(
            "distributions", {}
        ).get("chromosome_stats")
        if not chrom_stats:
            continue
        # Handle both formats:
        #   dict keyed by chromosome: {"chr1": {stats...}, "chr2": ...}
        #   list of dicts: [{"chromosome": "chr1", ...}, ...]
    ...
    # Determine numeric fields from first entry
    numeric_fields = set()
