...
class BedBaseConfig(object):
    ...
    def _init_qdrant_backend(self) -> QdrantBackend:
        ...
        try:
            ...
            _LOGGER.error(f"error in Connection to qdrant! skipping... Error: {err}. Qdrant host: {self._config.qdrant.host}")
            warnings.warn(
    ...
    def _init_qdrant_text_backend(self) -> Union[QdrantBackend, None]:
        ...
        try:
            ...
            _LOGGER.error(f"Error in Connection to qdrant text! skipping {e}. Qdrant host: {self._config.qdrant.host}")
            warnings.warn(
